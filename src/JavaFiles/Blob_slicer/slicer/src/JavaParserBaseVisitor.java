...
public class JavaParserBaseVisitor<T> extends AbstractParseTreeVisitor<T> implements JavaParserVisitor<T> {
	...
	@Override public T visitCompilationUnit(JavaParser.CompilationUnitContext ctx) {
		slicer.method_in_class_counter=0;
	...
	@Override public T visitMethodDeclaration(JavaParser.MethodDeclarationContext ctx) {
		...
		slicer.method_in_class_counter++;
		slicer.methodTemporaryContext="";
		slicer.methodName="";
		slicer.methodTemporaryContext= ctx.getText();
		slicer.methodName=ctx.IDENTIFIER().toString();
